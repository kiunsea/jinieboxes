<%
  //현재 구현한 jsp 코드와 html 코드에 부조화 때문인지 출력이 올바르지 않다
  //html 태그 닫힘이 올바르지 않은 것 같다
  //유지보수 차원에서 현재 구현한 페이지는 백업하고 javascript 만으로 ui를 디자인한다
%>
...
<head>
    ...
    <title>JINIEBOX 나눔아이템 목록</title>

    <link rel="canonical" href="https://getbootstrap.com/docs/5.3/examples/dashboard/">

    <link href="./res/web/bootstrap/css/bootstrap.min.css" rel="stylesheet"
    ...
    <link href="./res/web/dashboard.css" rel="stylesheet">
...
<body>
    ...
    <div class="dropdown position-fixed bottom-0 end-0 mb-3 me-3 bd-mode-toggle">
        ...
        <!-- 리스트 상단 -->
        <button type="button" class="btn btn-outline-primary py-2 d-flex" id="btn_list_top">
...
<%
    ...
    while (itemsIter.hasNext()) {
        JSONObject item = (JSONObject) itemsIter.next();
        //out.print("item_"+item.get("name")+"="+item.get("qty")+"<br/>");
        if (item.get("imgs") != null) {
            List<String> itmImgs = (List) item.get("imgs");
%>
...
<%
            if (itmImgs.size() > 0) {
%>
                                        <div id="box_imgs_<%=itemCnt%>" class="carousel slide">
                                            <div class="carousel-inner">
<%
                Iterator imgIter = itmImgs.iterator();
                int imgCnt = 0;
                while(imgIter.hasNext()) {
                    String imgName = imgIter.next().toString();
                    String ciclass = "carousel-item";
                    if (imgCnt == 0) ciclass += " active";
%>
                                                <div class="<%=ciclass%>">
                                                    <img src="<%=imgName%>" class="d-block w-100" alt="...">
                                                </div>
<%
                    imgCnt++;
                }
%>
                                            ...
                                            <button class="carousel-control-prev" type="button" data-bs-target="#box_imgs_<%=itemCnt%>" data-bs-slide="prev">
                                                <span class="carousel-control-prev-icon" aria-hidden="true"/>
                                                <span class="visually-hidden">Previous</span>
                                            </button>
                                            <button class="carousel-control-next" type="button" data-bs-target="#box_imgs_<%=itemCnt%>" data-bs-slide="next">
...
%>
    ...
    <footer id="jb_footer" class="my-3 text-center text-small"></footer>
    <script>importHTML("/jbs/jb_footer.html", 'jb_footer');</script>
    <script src="./res/web/bootstrap/js/bootstrap.bundle.min.js"
        integrity="sha384-ENjdO4Dr2bkBIFxQpeoTz1HIcje39Wm4jDKdf19U8gI4ddQ3GYNS7NTKfAdVQSZe"
        crossorigin="anonymous"></script>

    <script src="./res/web/feather.min.js"
